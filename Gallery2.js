document.addEventListener("DOMContentLoaded", function() {
    ...
    const numImagenes = 13; // Número total de imágenes
    const nombresImagenes = Array.from({ length: numImagenes }, (_, i) => i + 1);

    nombresImagenes.forEach((i) => {
        const listItem = document.createElement("li");
        const anchor = document.createElement("a");
        anchor.setAttribute("href", `#image-${i}`);
        const image = document.createElement("img");
        image.setAttribute("src", `images/thumbs/thumbsEspejos/${i}.jpg`);
        ...
        fullImage.setAttribute("src",`images/full/fullEspejos/${i}.jpg`);
